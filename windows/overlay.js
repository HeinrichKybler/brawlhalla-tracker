...
function legendChips(side, currentLegend, weaponMap) {
  ...
  return legs.map(l => {
    ...
    return `<div class="chip ${isCur ? 'cur' : ''}">
      <div class="cn">${A.imgTag(A.legendIcon(l.name, l.id), 18)}<span class="t">${l.name || '?'}</span>${isCur ? '<span class="star">★</span>' : ''}</div>
...
function weaponChips(side, currentLegend, weaponMap) {
  ...
  return weapons.map(w => {
    ...
    return `<div class="chip ${isCur ? 'cur' : ''}">
      <div class="cn">${A.imgTag(A.weaponIcon(w.weapon), 16)}<span class="t">${w.weapon}</span></div>
      <div class="cbar"><span style="width:${pct}%"></span></div>
    </div>`;
  }).join('');
}
  ...
  el.innerHTML = `
    <div class="nmrow">
      ...
      <span class="er"><span class="elo">${r.rating != null ? r.rating : '—'}</span>
        ${A.imgTag(A.getRankIcon(r.tier), 20, 'rankico')}</span>
    </div>
